...
throw @'
This legacy seed expansion script has been disabled.

Reason:
- assets/seed/initial_question_pool.json is now a curated source of truth.
- Running this script would overwrite the curated pool with older expansion content.
...
$readingComprehensionExpansion = @(Get-UpcatReadingComprehensionBank)
...
$questionBank = @{
  'Reading Comprehension' = @($readingComprehensionExpansion | ForEach-Object { @{ q = "$(($_.q -replace '^Passage:\s*', '')) Extra RC"; c = $_.c; d = $_.d; e = $_.e } })
  'Language Proficiency' = @(
    ...
    @{ q = 'Which sentence correctly uses commas to set off a nonessential clause?'; c = 'My brother, who lives in Cebu, will visit us next week.'; d = @('My brother who lives in Cebu will visit us, next week.', 'My brother who lives in Cebu, will visit us next week.', 'My brother, who lives in Cebu will visit us next week.'); e = 'The nonessential clause who lives in Cebu should be enclosed with commas.' },
  ...
  'Mathematics' = @(
    ...
    @{ q = 'Compute 2/3 + 1/6.'; c = '5/6'; d = @('3/6', '1', '4/9'); e = 'Convert 2/3 to 4/6, then add 4/6 + 1/6 = 5/6.' },
    ...
    @{ q = 'Evaluate 2^3 x 2^4.'; c = '128'; d = @('16', '32', '64'); e = 'When multiplying powers with the same base, add the exponents: 2^(3+4) = 2^7 = 128.' },
    ...
    @{ q = 'Which statement gives the divisibility rule for 9?'; c = 'the sum of its digits is divisible by 9'; d = @('it ends in 9', 'it is an odd number', 'its last two digits form a multiple of 9'); e = 'The divisibility rule for 9 depends on the sum of the digits.' },
    ...
    @{ q = 'Find the product of 2.5 and 0.4.'; c = '1.0'; d = @('0.5', '0.8', '10.0'); e = 'Multiply 25 by 4 to get 100, then place two decimal places to get 1.0.' },
